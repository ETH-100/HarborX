#!/usr/bin/env python3
# report.py -- one-click window-size benchmark and Markdown report (cross-platform)
...
def run_duckdb_live(arrow_snapshot:str, recent_arrows:list)->float:
    import duckdb, pyarrow.ipc as ipc, time as _t
    con = duckdb.connect()
...
def main():
    ...
    # Markdown
    lines = []
    lines.append("# HarborX Window-Size Benchmark Report")
    lines.append("")
    lines.append(f"- Total rows: **{total:,}** ({args.parts}×{args.rows:,})  ")
    lines.append(f"  Update ratio: **{args.update_ratio:.2f}** → keyspace≈**{K:,}**")
    ...
    for W in windows:
        ...
        # Arrow live query
        recent = [str(p).replace("\\","/") for p in sorted(Path("lake/hot").glob("*.arrow"))
                  if int(Path(p).stem.split("_")[-1]) > base_max]
